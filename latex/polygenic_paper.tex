...
\let\thefootnote\relax\footnotetext{jesse.murray@stats.ox.ac.uk}
    ...
    for j in range(len(matrix) - 1, -1, -1):
        ...
        for a, b, c in zip(step_labels, values_sum_list, [value for value in matrix[j]]):
            if c >= 0.01:
                ...
                if i > 3:
                    round_str = "{:0.1%}"
                ax.text(a, num, ' ' + round_str.format(c), va='bottom', ha='center', 
                		color=color, size=13, alpha=0.8)
...
## Example of using the functions: producing the percentile transitions for 
...
for i in range(m):
    matrices_m[i] = pk.get_matrix(r=rv[i], rs=rsv[i], n=1, num_iters=1_000_000,
    				low_round=6, n_subs0=5, n_subs1=5)
